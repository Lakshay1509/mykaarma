...
- [x] 👤 Design diagram — export §3 of the design doc *(Excalidraw or Mermaid)* *(Mermaid, in the README's "How it works", so GitHub renders it and it diffs as text. Standby, read replica and real provider adapters are dashed: designed, not built. Rendered with mermaid-cli to check the layout; the lifecycle diagram in `docs/design-decisions.md` too)*
